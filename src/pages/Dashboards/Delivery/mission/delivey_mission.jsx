        ...
        <div className="space-y-6">
          ...
          <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
            ...
            <div className="flex flex-wrap gap-2 sm:gap-3">
              <button
                ...
                className={`px-3 sm:px-4 py-2 rounded-xl flex items-center gap-2 transition-all duration-200 ${
                  showHeatmap 
              ...
              <button className="px-3 sm:px-4 py-2 bg-gradient-to-r from-green-500 to-emerald-500 text-white rounded-xl flex items-center gap-2 hover:from-green-600 hover:to-emerald-600 transition-all duration-200 shadow-lg hover:shadow-xl">
                <Zap size={16} className="sm:size-[18px]" />
                <span className="hidden xs:inline">Mode Boost</span>
              </button>
            </div>
          </div>
            ...
            <div className="bg-white dark:bg-gray-800 rounded-2xl p-4 sm:p-6 shadow-lg border border-gray-200 dark:border-gray-700">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-4">
                {heatmapZones.map((zone, index) => (
                  <div 
                    key={index}
                    className="p-3 sm:p-4 rounded-xl border-2 border-dashed transition-all duration-300 hover:scale-[1.02] cursor-pointer"
                    style={{
                  ...
                  >
                    <div className="flex items-center justify-between mb-2">
                      <h4 className="text-sm sm:text-base font-medium text-gray-900">{zone.name}</h4>
                      <Target size={14} className={
                        zone.demand > 80 ? 'text-red-500' : zone.demand > 60 ? 'text-yellow-500' : 'text-green-500'
                      } />
                    </div>
                    <div className="text-xl sm:text-2xl font-bold text-gray-900">{zone.demand}%</div>
                    <div className="text-xs sm:text-sm text-gray-600">Demande</div>
                  </div>
                ))}
              </div>
            </div>
          ...
          <div className="bg-white dark:bg-gray-800 rounded-2xl p-4 sm:p-6 shadow-lg border border-gray-200 dark:border-gray-700">
            <div className="flex flex-col lg:flex-row gap-3 sm:gap-4">
              ...
              <div className="flex bg-gray-100 dark:bg-gray-700 rounded-xl p-1 overflow-x-auto">
                {[
                  ...
                  >
                    {tab.label}
                    <span className="bg-gray-200 dark:bg-gray-600 text-[10px] sm:text-xs px-1.5 sm:px-2 py-0.5 sm:py-1 rounded-full">
                      {tab.count}
                    </span>
                  </button>
                ))}
              ...
              <div className="flex flex-1 flex-col sm:flex-row gap-2 sm:gap-3">
                <div className="relative flex-1">
                  ...
                  <input
                    ...
                    className="w-full pl-9 sm:pl-10 pr-3 sm:pr-4 py-2 border border-gray-300 dark:border-gray-600 rounded-xl bg-white dark:bg-gray-700 text-gray-900 dark:text-white placeholder-gray-500 focus:ring-2 focus:ring-orange-500 focus:border-transparent text-sm sm:text-base"
                  />
                </div>
                ...
                <select
                  ...
                  className="px-3 sm:px-4 py-2 border border-gray-300 dark:border-gray-600 rounded-xl bg-white dark:bg-gray-700 text-gray-900 dark:text-white focus:ring-2 focus:ring-orange-500 focus:border-transparent text-sm sm:text-base"
                >
          ...
          <div className="bg-gradient-to-br from-blue-50 to-indigo-100 dark:from-gray-800 dark:to-gray-900 rounded-2xl p-4 sm:p-6 border border-blue-200 dark:border-gray-700">
            ...
            <div className="text-center py-4 sm:py-8">
              ...
              <div className="text-base sm:text-lg font-medium text-gray-900 dark:text-white mb-1 sm:mb-2">
          ...
          <div className="space-y-3 sm:space-y-4">
            {filteredMissions.length === 0 ? (
              <div className="bg-white dark:bg-gray-800 rounded-2xl p-6 sm:p-12 text-center shadow-lg border border-gray-200 dark:border-gray-700">
...
const StatsCard = ({ icon, title, value, trend, trendColor }) => (
  <div className="bg-white dark:bg-gray-800 rounded-2xl p-4 sm:p-6 shadow-lg border border-gray-200 dark:border-gray-700 hover:shadow-xl transition-all duration-200 hover:scale-[1.02]">
    <div className="flex items-center justify-between mb-3 sm:mb-4">
      <div className="p-1 sm:p-2 bg-gray-100 dark:bg-gray-700 rounded-xl">
        {icon}
      </div>
...
const MissionCard = ({ mission, onAccept, onReject, onComplete, loading, isDarkMode }) => {
  ...
  return (
    <div className={`bg-white dark:bg-gray-800 rounded-2xl p-4 sm:p-6 shadow-lg border-2 transition-all duration-300 hover:shadow-xl ${getPriorityColor(mission.priority)}`}>
      {/* Header - Responsive */}
      <div className="flex flex-col sm:flex-row sm:items-start justify-between mb-3 sm:mb-4 gap-3">
        <div className="flex items-start gap-3 sm:gap-4">
          <div className="p-2 sm:p-3 bg-gradient-to-r from-orange-500 to-red-500 rounded-xl text-white shadow-lg">
            {mission.type === 'batch' ? <Users size={16} className="sm:size-[20px]" /> : <Package size={16} className="sm:size-[20px]" />}
          </div>
          ...
          <div className="flex-1">
            <div className="flex flex-wrap items-center gap-1 sm:gap-2 mb-1">
              ...
              <span className={`px-2 py-1 rounded-full text-[10px] sm:text-xs font-medium ${getStatusColor(mission.status)}`}>
                {mission.status === 'available' ? 'Disponible' : 
                 mission.status === 'in_progress' ? 'En cours' : 'Terminé'}
              </span>
              {mission.type === 'batch' && (
                <span className="px-2 py-1 rounded-full text-[10px] sm:text-xs font-medium bg-purple-100 text-purple-800 dark:bg-purple-900/20 dark:text-purple-400">
        ...
        <div className="flex items-center justify-between sm:justify-end gap-2">
          ...
          >
            <MoreVertical size={14} className="sm:size-[16px] text-gray-400" />
          </button>
        </div>
      </div>
      ...
      {expanded && (
        <div className="space-y-3 sm:space-y-4 mt-3 sm:mt-4 pt-3 sm:pt-4 border-t border-gray-200 dark:border-gray-700">
          {mission.customer && (
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 sm:gap-4">
              ...
              <div className="flex items-center gap-1 sm:gap-2">
                <button className="p-1 sm:p-2 bg-green-100 dark:bg-green-900/20 text-green-600 dark:text-green-400 rounded-lg hover:bg-green-200 dark:hover:bg-green-900/40 transition-colors">
                  <Phone size={14} className="sm:size-[16px]" />
                </button>
                <button className="p-1 sm:p-2 bg-blue-100 dark:bg-blue-900/20 text-blue-600 dark:text-blue-400 rounded-lg hover:bg-blue-200 dark:hover:bg-blue-900/40 transition-colors">
      ...
      {/* Actions - Responsive */}
      <div className="flex flex-col sm:flex-row items-center gap-2 sm:gap-3 mt-3 sm:mt-4">
        {mission.status === 'available' && (
          <>
            <button
              onClick={() => onAccept(mission.id)}
              disabled={loading}
              className="w-full sm:flex-1 bg-gradient-to-r from-green-500 to-emerald-500 hover:from-green-600 hover:to-emerald-600 text-white py-2 sm:py-3 px-3 sm:px-4 rounded-xl font-medium transition-all duration-200 flex items-center justify-center gap-1 sm:gap-2 shadow-lg hover:shadow-xl hover:scale-[1.02] disabled:opacity-50 disabled:cursor-not-allowed text-sm sm:text-base"
            >
            ...
            <button
              ...
              className="w-full sm:w-auto px-3 sm:px-4 py-2 sm:py-3 border border-gray-300 dark:border-gray-600 text-gray-700 dark:text-gray-300 rounded-xl hover:bg-gray-50 dark:hover:bg-gray-700 transition-colors flex items-center justify-center gap-1 sm:gap-2 text-sm sm:text-base"
            >
        ...
        {mission.status === 'in_progress' && (
          <div className="w-full flex flex-col sm:flex-row items-center gap-2 sm:gap-3">
            <button className="w-full sm:flex-1 bg-gradient-to-r from-blue-500 to-indigo-500 hover:from-blue-600 hover:to-indigo-600 text-white py-2 sm:py-3 px-3 sm:px-4 rounded-xl font-medium transition-all duration-200 flex items-center justify-center gap-1 sm:gap-2 shadow-lg hover:shadow-xl hover:scale-[1.02] text-sm sm:text-base">
              <Navigation size={14} className="sm:size-[18px]" />
              Navigation
            </button>
            <button
        ...
        {mission.status === 'completed' && (
          <div className="w-full text-center py-1 sm:py-2 text-green-600 dark:text-green-400 font-medium text-sm sm:text-base">
            ✅ Mission terminée
          </div>
        )}
      </div>
